...
#define PPM_LIGHT_CHANNEL      6
#define PPM_DOCKING_CHANNEL    7
#define PPM_SWITCH_CHANNEL     8
#define PPM_CONTROL1A_CHANNEL  1
#define PPM_CONTROL1B_CHANNEL  2
#define PPM_CONTROL2A_CHANNEL  3
#define PPM_CONTROL2B_CHANNEL  4
#define PPM_WHEEL1_CHANNEL     9
#define PPM_WHEEL2_CHANNEL     10
...
void loop() {
  ...
  // Свет
  bool lightEnabled = readTwoStatePpm(PPM_LIGHT_CHANNEL) == 1;
  digitalWrite(LIGHT_PIN, lightEnabled ? HIGH : LOW);

  // Переключение ног
  previousLeg = currentLeg;
  currentLeg = readTwoStatePpm(PPM_SWITCH_CHANNEL);
  ...
  int dockingMode = readThreeStatePpm(PPM_DOCKING_CHANNEL);
  if (dockingModes[currentLeg] != dockingMode) {

    // Устанавливаем режим стыковки для текущей сервы
    dockingModes[currentLeg] = dockingMode;
    setServoDockingMode(legs[currentLeg], dockingModes[currentLeg], currentLeg == 1);
    
    // И отключаем предыдущую ногу
    //dockingModes[previousLeg] = DOCKING_MODE_NEUTRAL;
    //setServoDockingMode(legs[previousLeg], dockingModes[previousLeg]);
  }

  // Хакнутое движение. Стики напрямую переводятся в углы

  // Сервы около платформы
  int _0 = readPpm(PPM_CONTROL1A_CHANNEL, 10, 80);
  legs[0]->a->setTargetDeg(_0);
  legs[0]->b->setTargetDeg(_0);

  int _1 = readPpm(PPM_CONTROL2A_CHANNEL, 10, 80);
  legs[1]->a->setTargetDeg(_1);
  legs[1]->b->setTargetDeg(_1);

  // Поворотные сервы
  int _r0 = readPpm(PPM_CONTROL1B_CHANNEL, 0, 160);
  legs[0]->r->setTargetDeg(_r0);

  int _r1 = readPpm(PPM_CONTROL2B_CHANNEL, 0, 160);
  legs[1]->r->setTargetDeg(_r1);
}
...
void setServoDockingMode(Leg *leg, uint8_t mode, bool hackingMode) {
  if (hackingMode) {
    ...
    if (mode == DOCKING_MODE_UNDOCK) {
      leg->d->setPWM(leg->pinD, 2000, 0);
    } else if (mode == DOCKING_MODE_NEUTRAL) {
      leg->d->setPWM(leg->pinD, 0, 0);
    } else if (mode == DOCKING_MODE_DOCK) {
      leg->d->setPWM(leg->pinD, 4000, 0);
    }
  }
}
